...
$nameErr = $emailErr = $genderErr = $passErr = $productErr = $phoneErr = $product_addressErr =$product_type= $key= $product_id="";
$name = $email = $gender = $comment = $pass= $product = $phone =$product_address= $product_typeErr =  $activateErr = $product_idErr="";
$flag="true";
$activate= 0;
$enable = "disabled";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty($_POST["password"])) {
    ...
    $key =  bin2hex(openssl_random_pseudo_bytes ( 6 ) ) . $product_id ;
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($flag) &&!empty($_POST["name"]) && !empty($email) && !empty($pass) && !empty($product) && !empty($phone) && !empty($product_address) && !empty($product_id) && !empty($product_type)   )
{
   
  $db = new MyDB();
  
  $sql =
  "INSERT INTO Records (name,email,pass,product,phone,product_id,product_address,product_type,key,activate ) values (' ".$name."','".$email."','".$pass."','".$product."',".$phone.",'".$product_id."','".$product_address."','".$product_type."','".$key."',".$activate.")";
...
<body>
  ...
  <span class="error"> <?php echo $activateErr;?></span>
          ...
          &nbsp;&nbsp;
                ...
                <a href="allrecords_test.php">Cancel</a>
            </div>
        </fieldset>
    </form><br/>
	<form  action="single_record_test.php">  
