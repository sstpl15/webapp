...
if(isset($_GET['addrs']))  $addrs="'".$_GET['addrs']."'";
  ...
  $sql =<<<EOF
	  ...
	  SELECT s.RowId as rowid, Address,MAC,s.Time as Time,freq,Modulation,Data_Rate,Code_Rate,RSSI,LORA_SNR,Frame_length,PAYLOAD from SSTPL_UP_Data as s inner join Records as r on s.Address= r.product_address  where s.Address=$addrs order by s.Time DESC;
EOF;
	...
	if(isset($_GET['to']) && isset($_GET['from']) )
	{
	  ...
	  SELECT s.RowId as rowid, Address,MAC,s.Time as Time,freq,Modulation,Data_Rate,Code_Rate,RSSI,LORA_SNR,Frame_length,PAYLOAD from SSTPL_UP_Data as s inner join Records as r on s.Address= r.product_address  where s.Address=$addrs AND s.Time BETWEEN $from AND $to order by s.Time DESC;
EOF;
		//echo $sql;
	}
