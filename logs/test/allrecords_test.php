<?php
  ...
  $sql =<<<EOF
      SELECT RowId,count(product_id) as count_addrs,* from Records where product in('LCD') group by email , product;
EOF;
  ...
  if(isset($_GET['from']) && isset($_GET['to']) )
  {
	  ...
	  $to="'".trim($_GET['to'])."'";
	  $sql =<<<EOF
    SELECT RowId,count(product_id) as count_addrs,* from Records where product in('LCD') group by email ;
EOF;
	 // echo $sql;
  }
...
EOF;
	...
	if(isset($_GET["val"]))
	{
		$i = $_GET['val'];
		$sql =<<<EOF
      SELECT RowId,count(product_id) as count_addrs,* from Records group by email , product order by $i;
	EOF;
  ...
  $sql =<<<EOF
      SELECT RowId,count(product_id) as count_addrs,* from Records group by email , product;
EOF;
   }
        ...
        <table class="table table-striped">
            <thead>
                ...
                <tr>
					...
					<th><a href="allrecords_test.php?val=name"><font style="color:white">Name</font</a></th>
						<th><a href="allrecords_test.php?val=email"><font style="color:white">Email</font></a></th>
					
						<th><a href="allrecords_test.php?val=product"><font style="color:white">Product</font></a></th>
						<th><a href="allrecords_test.php?val=phone"><font style="color:white">Phone</font></a></th>
					
					
					
				
						<th><a href="allrecords_test.php?val=time"><font style="color:white">Time</font></a></th>
			...
			?>    
                    <tr onclick="document.location = 'allrecords1_test.php?email=<?php echo $row['email'] ?> & product=<?php echo $row['product'] ?> ';">
